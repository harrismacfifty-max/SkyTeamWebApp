...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Ausbilder | Anmeldung (Webapp) | Schüler oder Schülerverwaltung | Login-Ansicht, Header-Loginstatus | `POST /api/auth/login`, `GET /api/auth/me`, `POST /api/auth/logout` | `AuthService`, `DemoAccountProvider` | In-Memory-Session, keine DB-Tabelle | Umgesetzt; `sc901` bis `sc907` = eigene Schülerkonten, `demo2` = `SCHUELERVERWALTUNG`, `demo` = nicht empfohlener Legacy-Alias für `SC901` |
